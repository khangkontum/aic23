...
def fuzzy_search(query, files, top):
    ...
    for file_path in files:
        ...
        for segment in segments:
            ...
            matches.append([file_path[-13:-5], text, ratio, segment['start'], segment['end']])

    return sorted(matches, key=lambda x: x[2], reverse=True)[:top]

def get_result_fuzzy_search(directory = "./transcript", query = "", top = 40):
...
def get_utube_link(model, video, timeStart):
    for sample in model.dataset:
        if sample["video"] == video:
            return sample["youtube_url"].split("&")[0] + "&t=" + str(int(timeStart))
    ...
    for file_path in files:
        ...
        for segment in segments:
            ...
            if re.search(norm_sub_query, norm_text):
                matches.append([file_path[-13:-5], text, segment['start'], segment['end']])

    return matches

def fulltext_search(directory = "./transcript", query = "", top=None):
    files = [os.path.join(directory, file) for file in os.listdir(directory) if file.endswith('.json')]
